...
if __name__ == "__main__":
    ...
    sc = SparkContext(appName="MNIST_CLF", conf=conf).getOrCreate()

    sql = SQLContext(sc)

    df = sql.read.format("com.databricks.spark.csv").options(header= True, inferSchema=True, sep=',').load("hdfs://localhost:9000/user/saidalikhonalikhonov/mnist/train.csv")
